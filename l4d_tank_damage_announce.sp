	...
	if (!bPrintedHealth)
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			if(g_iWasTank[i] > 0)
			{
				...
				CPrintToChatAll("{default}[{green}★{default}] {red}Damage {default}dealt to {olive}Tank {default}({red}%s{default}) {default}[{green}★{default}]", name);
				g_iWasTank[i] = 0;
			}
			else if(g_iWasTankAI > 0) 
			{
				decl String:name[MAX_NAME_LENGTH];
				GetClientName(i, name, sizeof(name));
				CPrintToChatAll("{default}[{green}★{default}] {red}Damage {default}dealt to {red}Tank {default}({olive}AI{default}) Last Ctrl : %s)",name);
				g_iWasTankAI = 0;
			}
		}
	}
